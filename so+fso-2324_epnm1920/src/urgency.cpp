...
void* nurse_main(void* arg) {
   int index = *(int *)arg;
   while (nurse_iteration(index) == 0);
   return NULL;
}

void* doctor_main(void* arg) {
   int index = *(int *)arg;
   while (doctor_iteration(index) == 0);
   return NULL;
}
...
int main(int argc, char *argv[])
{
   ...
   for (int i = 0; i < npatients; i++) {
      p_id[i] = i;
      thread_create(&p[i], NULL, &patient_main, (void *)&p_id[i]);
   }

   for (int i = 0; i < nnurses; i++) {
      n_id[i] = i;
      thread_create(&n[i], NULL, &nurse_main, (void *)&n_id[i]);
   }

   for (int i = 0; i < ndoctors; i++) {
      d_id[i] = i;
      thread_create(&d[i], NULL, &doctor_main, (void *)&d_id[i]);
   }
   ...
   for (int i = 0; i < nnurses; i++) {
      thread_join(n[i], NULL);
   }

   for (int i = 0; i < ndoctors; i++) {
      thread_join(d[i], NULL);
   }
   /* end of dummy code */
